...
TURSO_RS_FEATURES=${TURSO_RS_FEATURES:-turso_core/fts}
...
CARGO_ARGS_ARR=(--locked --profile "$TURSO_RS_BUILD_PROFILE")
if [[ -n "$TURSO_RS_FEATURES" ]]; then
  CARGO_ARGS_ARR+=(--features "$TURSO_RS_FEATURES")
fi
